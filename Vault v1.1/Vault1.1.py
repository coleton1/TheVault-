...
def show_password_analysis_window(credentials):
    ...
    # Add a textbox to display the analysis results
    ...
    # Highlight weak feedback in red
    analysis_textbox.tag_config("weak", foreground="red")
    # Highlight strong feedback in green
...
def vault_gui():
    ...
    image_path = "vaultimage2.png"  # Replace with your image file path
    vault_image = ctk.CTkImage(Image.open(image_path), size=(125, 125))  # Adjust size as needed
    
    # Add the image to a CTkLabel and place it in the middle
    ...
    def show_credentials():
        credentials = load_passwords(key)
        credentials_textbox.delete("1.0", ctk.END)  # Clear the textbox
        for website, username, password in credentials:
    ...
    analyze_button = ctk.CTkButton(app, text="Analyze Passwords", command=lambda: show_password_analysis_window(load_passwords(key)))
...
def login_gui():
    ...
    """hiding submit button for now because I dont need it for now but possibly will reuse"""
    #submit_button = ctk.CTkButton(login_window, text="Submit", command=check_password)
...
def info_gui():
    #this is gonna be a window that pops up when you hit the info button
    #it will display either all protocols and algorithems used to create this program 
     #or it can display your vault info such as how many passwords, password strength etc.
    info_gui = ctk.CTk()
    info_gui.geometry("500x600")
    info_gui.title("Information")
    info_gui.resizable(False,False)

    ctk.CTkLabel(info_gui,text="This is a placeholder for information that will be here, at some point.",font=("Roboto",12,"bold")).pack()
